...
int main(int argc, char *argv[]) {
	...
	while (1) {
		// allocate memory for pointer u and uhelp
		double *u = param.u, *uhelp = param.uhelp, *uswap;
		int blocksize = 150;
		...
		while (1) {
			...
			case 0: // JACOBI
				...
				residual = relax_jacobi_mix(param.u, param.uhelp, np, np);
				residual = relax_jacobi_mix_blocking(param.u, param.uhelp, np, np, blocksize);
				// swap matrices; uhelp is used on next iteration
				uswap = u;
    			u = uhelp;
    			uhelp = uswap;
				break;
